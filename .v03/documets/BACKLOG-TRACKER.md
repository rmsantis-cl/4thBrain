...
| **Working Notes** | [[story-1.3.md](./story/story-1.3.md)] — includes live evidence (job 30) and the full source map. ADR: [[adr24-actuator-coordinator.md](./design/adr24-actuator-coordinator.md)]. Design debt: [[DESIGN-DEBT.md](./DESIGN-DEBT.md)] item 7. Bug that surfaced the gap: [[Bug-101.md](./bugs/Bug-101.md)]. |
...
| Field | Value |
|---|---|
| **Abstract** | Real-time visibility into document ingestion progress through the job queue and executor (job actuator) pipeline. |
| **Description** | When users submit text/file/URL via the ingestion form, they receive a jobId but have no visibility into what happens next. This story adds an admin panel ("Actuators") that provides end-to-end traceability through the ingestion pipeline. Users can submit a document, get its jobId, then use this panel to track it through all five executors (ingest, convert, classify, index, briefing) — seeing which stages it's passed through, where it ended up in the vault (if successful), and error details if it failed. The panel also provides queue health monitoring: at a glance, see how many jobs are pending/running/completed/failed for each executor type, to identify bottlenecks or failures. Backend: new `/api/actuators/status` endpoint (read-only) returns job counts and recent failed jobs grouped by executor type. Frontend: new "Actuators" nav item that opens a panel with executor queue cards (mirroring the stat-grid layout from the Status panel) and a search box to track individual documents by jobId or URI. Dev-mode protected. See ADR23 for full design. |
| **Dependencies** | depends on Story 13.3 (repository layer, `/api/tables/*` API, job repository methods), depends on Story 6.4 (UI shell) |
| **Acceptance Criteria** | • New "Actuators" nav item in sidebar, opens a dedicated panel (not a sub-menu).<br>• Backend `/api/actuators/status` endpoint (GET, dev-mode only) returns JSON with executor queues: `{ executors: { ingest: {pending, running, completed, failed, failedJobs}, ... }, lastUpdated: "..." }`.<br>• Frontend displays 5 executor queue cards (one per executor type: ingest, convert, classify, index, briefing) showing pending/running/completed/failed counts with consistent color badges (blue=running, green=completed, red=failed, neutral=pending).<br>• Search/filter box allows users to enter a jobId or document URI; UI displays the document's current status and job progression (which executors touched it, start/end dates for each).<br>• Final vault location shown for indexed documents (path in `document.topic` field).<br>• Failed jobs display error message (from `job.error_message` column, integrated in Story 6.3).<br>• Refresh-on-demand button to pull latest queue state (no automatic polling in v0.1).<br>• Panel styling consistent with existing admin panels: uses stat-grid/stat-card for executor cards, same color palette and button styles, same section headings and layout patterns as Status panel (Story 6.3) and Search panel (Story 6.2).<br>• Dev-mode protected (same as `/admin/db` and `/api/docs` — returns 403 when NODE_ENV != development).<br>• No schema changes required (uses existing `job`, `document`, `status`, `job_status`, `job_type` tables). |
| **Status** | READY |
| **Design** | [[ADR23](./adr23-actuator-queue-monitoring.md)] — Complete design spec covering backend endpoint, frontend panel layout, data model queries, UI consistency rules, and acceptance criteria mapping. |
