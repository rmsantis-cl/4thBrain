...
| documents/bug/BUG-001.md | [2026-09-07] Web UI fails to load on localhost:8080 or localhost:8080/chat; UIController routing or static resources issue; blocks P1.13 and P3.4<br>[2026-09-07] Diagnosed against the code: UIController and AdminController are `@Controller`s returning view names, and build.gradle carries no template engine, so the names resolve to nothing; `@GetMapping("/")` also suppresses the welcome page that would otherwise serve static/index.html. Missing CSS/JS and static-resource config ruled out. Three fix options recorded, none chosen |
