...
- Before writing or editing any code, the work must trace to:
  1. An existing **Epic** (`EPn`) and **Story** (`Story n.n`) in `PROJECT_4thBrain.md`, and
  2. A design that covers what's being built — an ADR, schema, class definition, or other design artifact under `documents/design/`, sufficient to implement from without improvising the shape of the solution while coding.
...
Tracked in `documents/DESIGN-DEBT.md`. When a gap is found mid-plan (a needed capability with no Epic/Story, or no design decision behind it):
