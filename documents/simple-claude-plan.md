...
Since this audit was taken, `INDEX.md` has been split into three scoped files — the repository
root, `documents/` and `scripts/` — so a story edit no longer reaches across the tree to touch the
root index. That addresses the per-edit cost, which was the sharper half of the problem. The
recommendation below rests on the other half, that git already records this, and is now a decision
to weigh rather than an obvious cleanup.
